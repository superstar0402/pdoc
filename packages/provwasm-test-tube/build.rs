...
fn main() {
    ...
    let prebuilt_lib_dir = manifest_dir.join("libprovwasmtesttube").join("artifacts");

    let lib_name = "provwasmtesttube";

    let out_dir = PathBuf::from(env::var("OUT_DIR").unwrap());

    let header = if std::env::var("DOCS_RS").is_ok() {
        manifest_dir
            .join("libprovwasmtesttube")
            .join("artifacts")
            .join("libprovwasmtesttube.docrs.h")
    } else {
        out_dir.join(format!("lib{}.h", lib_name))
    };
    // rerun when go code is updated
    println!("cargo:rerun-if-changed=./libprovwasmtesttube");
    ...
    if env::var("PROFILE").unwrap() == "debug" {
        ...
        // for each file with pattern `libprovwasmtesttube.*`, copy to target dir
        for entry in std::fs::read_dir(out_dir.clone()).unwrap() {
            let entry = entry.unwrap();
            let path = entry.path();
            if path.is_file() {
                let file_name = path.file_name().unwrap().to_str().unwrap();
                if file_name.starts_with("libprovwasmtesttube") {
...
fn build_libprovwasmtesttube(out: PathBuf) {
