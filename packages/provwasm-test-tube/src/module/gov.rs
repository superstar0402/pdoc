...
/// It has ability to access to `ProvwasmTestApp` which is more specific than `Runner`
pub struct GovWithAppAccess<'a> {
    gov: Gov<'a, ProvwasmTestApp>,
    app: &'a ProvwasmTestApp,
}

impl<'a> GovWithAppAccess<'a> {
    pub fn new(app: &'a ProvwasmTestApp) -> Self {
        Self {
            gov: Gov::new(app),
            app,
        }
    ...
    pub fn to_gov(&self) -> &Gov<'a, ProvwasmTestApp> {
        &self.gov
    }
